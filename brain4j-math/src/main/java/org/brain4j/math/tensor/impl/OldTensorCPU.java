...
public class OldTensorCPU implements Cloneable, Tensor {
    ...
    public OldTensorCPU(int[] shape, int[] strides) {
        if (shape.length == 0) {
            throw new IllegalArgumentException("Shape cannot be empty");
        }
    ...
    public OldTensorCPU(int[] shape, int[] strides, float[] data) {
        if (shape.length == 0) {
            throw new IllegalArgumentException("Shape cannot be empty");
        }
    ...
    public static Tensor of(int[] shape, int[] strides, float... data) {
        return new OldTensorCPU(shape, strides, data);
    }
    ...
    public static Tensor ones(int... shape) {
        Tensor tensor = new OldTensorCPU(shape);
    ...
    private static Tensor random(Random random, int... shape) {
        ...
        for (int i = 0; i < tensor.data().length; i++) {
            tensor.data()[i] = random.nextFloat();
        }
    ...
    private static Tensor uniform(Random random, double lowerBound, double upperBound, int... shape) {
        ...
        for (int i = 0; i < tensor.data().length; i++) {
            tensor.data()[i] = (float) (random.nextDouble() * range + lowerBound);
        }
    ...
    public static Tensor randn(Random random, double mean, double stddev, int... shape) {
        ...
        for (int i = 0; i < tensor.data().length; i++) {
            tensor.data()[i] = (float) (random.nextGaussian() * stddev + mean);
        }
    ...
    public Tensor permute(int... dims) {
        ...
        Tensor result = new OldTensorCPU(newShape);
    ...
    public Tensor matmul(Tensor other) {
        ...
        Tensor result = new OldTensorCPU(resultShape);
    ...
    public Tensor pow(Tensor other) {
        ...
        for (int i = 0; i < data.length; i++) {
            data[i] = (float) Math.pow(data[i], other.data()[i]);
        }
